...
function HangmanData() {
	...
	this.startTime = null;
	this.endTime = null;
	this.imageSetURL = "http://thiagi.com/hangman/balloon/";
...
function refreshImage(OuterDiv) {
	...
	elem.setAttribute("src", Hdata.imageSetURL+"hang_" + imgNo + ".gif");
	if (layout === "regular") {
...
function refreshHangmanApp(div_id){
	hangmanApp(div_id,map[div_id].questionString, map[div_id].answerString,map[div_id].count, map[div_id].layout, map[div_id].imageSetURL);
}

function hangmanApp(div_id, questions, answers, count, layout,imageSetURL) {
    ...
    if(imageSetURL!=undefined){
       setData.imageSetURL = imageSetURL;
    }
